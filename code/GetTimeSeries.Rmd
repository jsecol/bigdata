...
**accept defaults (but I used techprod_me)
*dont run, but tested to post all examples*
bin/post -c techproducts example/exampledocs/*

*example query in solr for my existing collection*
http://localhost:8983/solr/techprods_me/select?q=*%3A*&rows=10&start=0&wt=csv

#### base R way
```{r}
library(curl)

url1 <- "http://localhost:8983/solr/techprods_me/select?q=*%3A*&rows=6&start=0&wt=csv"

cl_con <- curl(url1)

tmp_df <- read.csv(cl_con, stringsAsFactors = FALSE)
tmp_df
```
...
if (!cli$collection_exists("y2010")) {
  cli$collection_create(name = "y2010")
}
...
cli$ping("techprods_me")

tmp <- cli$search(name = "techproducts", list(q = "*:*"))

tmp <- solr_search(cli, "y2010", params = list(q='*:*', rows = 20000))

tmp <- solr_search(cli, "y2010", params = list(q='V3:TMAX', rows = 200))
